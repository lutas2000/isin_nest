      ...
      <div class="overflow-hidden rounded-lg bg-white shadow">
        <TableHeader title="員工列表">
          <template #actions>
            ...
            <button
              type="button"
              class="btn btn-outline inline-flex h-10 w-10 shrink-0 items-center justify-center p-0"
              :aria-pressed="hideSensitiveInfo"
              :aria-label="hideSensitiveInfo ? '顯示敏感資訊' : '隱藏敏感資訊'"
              :title="hideSensitiveInfo ? '顯示敏感資訊' : '隱藏敏感資訊'"
              @click="hideSensitiveInfo = !hideSensitiveInfo"
            >
              <svg
                v-if="hideSensitiveInfo"
                class="h-5 w-5"
...
const editingStaffTarget = ref<Staff | null>(null);
...
const visibleTableColumns = computed(() =>
  hideSensitiveInfo.value
    ? tableColumns.filter((col) => col.key !== 'wage')
    : tableColumns,
);
...
const editStaff = (staff: Staff) => {
  editingStaffTarget.value = staff;
