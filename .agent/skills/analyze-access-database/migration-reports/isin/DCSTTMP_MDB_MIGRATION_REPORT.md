# dcsttmp.mdb — Legacy Access 遷移盤點報告

| 項目 | 內容 |
| --- | --- |
| **權威檔案路徑** | `/nas/isin/dcsttmp.mdb`（庫存「同名檔主檔判定」主檔相對路徑：`dcsttmp.mdb`；歷史盤點檔名對應 `isin/dcsttmp.mdb`） |
| **盤點時間 (UTC)** | 2026-05-15 |
| **分析方式** | `npm run analyze-access -- "/nas/isin/dcsttmp.mdb" --json --no-samples`；含樣本：`--json --sample-offset=0 --sample-limit=15`；字串經腳本 Big5→UTF-8 轉換。 |
...
| `CNC3` | text | 12 | Y | varchar(12) | 無樣本列，需對照程式 |
| `CNC4` | text | 12 | Y | varchar(12) | 無樣本列，需對照程式 |
| `CNC5` | text | 50 | Y | varchar(50) | 無樣本列，需對照程式 |
...
```bash
npm run analyze-access -- "/nas/isin/dcsttmp.mdb" --json --no-samples
npm run analyze-access -- "/nas/isin/dcsttmp.mdb" --json --sample-limit=20
```
