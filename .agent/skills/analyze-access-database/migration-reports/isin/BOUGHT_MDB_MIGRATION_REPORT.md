# bought.mdb — Legacy Access 遷移盤點報告

| 項目 | 內容 |
| --- | --- |
| **權威檔案路徑** | `/nas/isin/bought.mdb`（庫存「同名檔主檔判定」主檔相對路徑：`bought.mdb`；歷史盤點檔名對應 `isin/bought.mdb`） |
| **盤點時間 (UTC)** | 2026-05-15 |
| **分析方式** | `npm run analyze-access -- "/nas/isin/bought.mdb" --json --no-samples`；含樣本：`--json --sample-offset=0 --sample-limit=15`；字串經腳本 Big5→UTF-8 轉換。 |
...
```bash
npm run analyze-access -- "/nas/isin/bought.mdb" --json --no-samples
npm run analyze-access -- "/nas/isin/bought.mdb" --json --sample-limit=20
```
