# acstation.mdb — Legacy Access 遷移盤點報告

| 項目 | 內容 |
| --- | --- |
| **權威檔案路徑** | `/nas/isin/acstation.mdb`（庫存「同名檔主檔判定」主檔相對路徑：`acstation.mdb`；歷史盤點檔名對應 `isin/acstation.mdb`） |
| **盤點時間 (UTC)** | 2026-05-15 |
| **分析方式** | `npm run analyze-access -- "/nas/isin/acstation.mdb" --json --no-samples`；含樣本：`--json --sample-offset=0 --sample-limit=15`；字串經腳本 Big5→UTF-8 轉換。 |
...
| --- | --- | ---: | --- | --- | --- |
| `xuser` | text | 15 | Y | varchar(15) | 不明（建議對照舊程式與業務） |
| `xcomputer` | text | 15 | Y | varchar(15) | 不明（建議對照舊程式與業務） |
| `xname` | text | 15 | Y | varchar(15) | 名稱 |
| `xdate` | text | 15 | Y | varchar(15) | 日期（可能為民國年文字） |
...
- **`computer`**：
  - 列 0: xuser=Q>Q7K862bCL:; xcomputer=U;H6;96:TE?4; xname=tW^]($"!@0("; xdate=@(I<?432PF>"
...
```bash
npm run analyze-access -- "/nas/isin/acstation.mdb" --json --no-samples
npm run analyze-access -- "/nas/isin/acstation.mdb" --json --sample-limit=20
```
