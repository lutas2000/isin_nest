# show.mdb — Legacy Access 遷移盤點報告

| 項目 | 內容 |
| --- | --- |
| **權威檔案路徑** | `/nas/isin/show.mdb`（庫存「同名檔主檔判定」主檔相對路徑：`show.mdb`；歷史盤點檔名對應 `isin/show.mdb`） |
| **盤點時間 (UTC)** | 2026-05-15 |
| **分析方式** | `npm run analyze-access -- "/nas/isin/show.mdb" --json --no-samples`；含樣本：`--json --sample-offset=0 --sample-limit=15`；字串經腳本 Big5→UTF-8 轉換。 |
...
| --- | --- | ---: | --- | --- | --- |
| `fd1` | long | 4 | Y | integer | 無樣本列，需對照程式 |
| `fd2` | text | 1 | Y | varchar(1) | 無樣本列，需對照程式 |
| `fd3` | text | 50 | Y | varchar(50) | 無樣本列，需對照程式 |
| `fd4` | text | 50 | Y | varchar(50) | 無樣本列，需對照程式 |
| `fd5` | text | 250 | Y | varchar(250) | 無樣本列，需對照程式 |
| `fd6` | text | 100 | Y | varchar(100) | 無樣本列，需對照程式 |
| `fd7` | text | 100 | Y | varchar(100) | 無樣本列，需對照程式 |
...
```bash
npm run analyze-access -- "/nas/isin/show.mdb" --json --no-samples
npm run analyze-access -- "/nas/isin/show.mdb" --json --sample-limit=20
```
