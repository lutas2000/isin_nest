# cust.mdb — Legacy Access 遷移盤點報告

| 項目 | 內容 |
| --- | --- |
| **權威檔案路徑** | `/nas/isin/cust.mdb`（庫存「同名檔主檔判定」主檔相對路徑：`cust.mdb`；歷史盤點檔名對應 `isin/cust.mdb`） |
| **盤點時間 (UTC)** | 2026-05-15 |
| **分析方式** | `npm run analyze-access -- "/nas/isin/cust.mdb" --json --no-samples`；含樣本：`--json --sample-offset=0 --sample-limit=15`；字串經腳本 Big5→UTF-8 轉換。 |
...
| `bank` | text | 20 | Y | varchar(20) | 不明（建議對照舊程式與業務） |
| `account` | text | 20 | Y | varchar(20) | 不明（建議對照舊程式與業務） |
| `credit` | currency | 8 | Y | numeric(18,4) | 不明（建議對照舊程式與業務） |
| `debt` | currency | 8 | Y | numeric(18,4) | 不明（建議對照舊程式與業務） |
| `deal_f` | text | 12 | Y | varchar(12) | 民國年日期文字 |
| `deal_l` | text | 12 | Y | varchar(12) | 民國年日期文字 |
| `dxfpath` | text | 50 | Y | varchar(50) | 無樣本列，需對照程式 |
| `email` | text | 50 | Y | varchar(50) | 無樣本列，需對照程式 |
...
- **`cust`**：
  - 列 0: code=01A; cutname=安邦; name=安邦金屬股份有限公司; master=黃茂榮; tel1=04-23112015; tel2= ; fax=04-23112017; zipno=406
  - 列 1: code=02A; cutname=安得; name=安得科技有限公司; master=馬永泰; tel1=04-2453981; tel2= ; fax=04-2419291; zipno=406
  - 列 2: code=02B; cutname=歐德利; name=歐德利車業; master=賴先生; tel1=04-24759171; tel2= ; fax= ; zipno= 
  - 列 3: code=0SA; cutname=安南; name=安南企業社; master= ; tel1=06-5976225; tel2= ; fax=06-5976301; zipno= 
  - 列 4: code=0XB; cutname=安利; name=安利鐵工廠; master= ; tel1=04-23954013; tel2=04-23952503; fax=04-23957146; zipno= 
...
```bash
npm run analyze-access -- "/nas/isin/cust.mdb" --json --no-samples
npm run analyze-access -- "/nas/isin/cust.mdb" --json --sample-limit=20
```
