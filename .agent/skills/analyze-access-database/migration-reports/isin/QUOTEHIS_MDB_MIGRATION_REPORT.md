# quotehis.mdb — Legacy Access 遷移盤點報告

| 項目 | 內容 |
| --- | --- |
| **權威檔案路徑** | `/nas/isin/quotehis.mdb`（庫存「同名檔主檔判定」主檔相對路徑：`quotehis.mdb`；歷史盤點檔名對應 `isin/quotehis.mdb`） |
| **盤點時間 (UTC)** | 2026-05-15 |
| **分析方式** | `npm run analyze-access -- "/nas/isin/quotehis.mdb" --json --no-samples`；含樣本：`--json --sample-offset=0 --sample-limit=15`；字串經腳本 Big5→UTF-8 轉換。 |
...
| `WORK` | text | 30 | Y | varchar(30) | 不明（建議對照舊程式與業務） |
...
- **`itable`**：
  - 列 0: FACTOR_NO=儀興科技; FACTOR= ; DATE_R= 101.08.13; DWG_REF=BRACKET-A; METAL=SPCC; THICK=1.2; WORK=未鍍鎳; QTY=10.0000
  - 列 1: FACTOR_NO=大普; FACTOR= ; DATE_R= 100.04.07; DWG_REF=TVMM-2000N-28; METAL=SUS304 2B; THICK=3; WORK= ; QTY=1.0000
  - 列 2: FACTOR_NO=大普; FACTOR= ; DATE_R= 100.04.07; DWG_REF=TVMM-2000N-29; METAL=SUS304 2B; THICK=3; WORK= ; QTY=1.0000
  - 列 3: FACTOR_NO=大普; FACTOR= ; DATE_R= 100.04.07; DWG_REF=TVMM-2000N-30; METAL=SUS304 2B; THICK=3; WORK= ; QTY=1.0000
  - 列 4: FACTOR_NO=大普; FACTOR= ; DATE_R= 100.04.07; DWG_REF=TVMM-2000N-31; METAL=SUS304 2B; THICK=3; WORK= ; QTY=1.0000
...
```bash
npm run analyze-access -- "/nas/isin/quotehis.mdb" --json --no-samples
npm run analyze-access -- "/nas/isin/quotehis.mdb" --json --sample-limit=20
```
