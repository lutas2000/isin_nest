# workout.mdb — Legacy Access 遷移盤點報告

| 項目 | 內容 |
| --- | --- |
| **權威檔案路徑** | `/nas/isin/workout.mdb`（庫存「同名檔主檔判定」主檔相對路徑：`workout.mdb`；歷史盤點檔名對應 `isin/workout.mdb`） |
| **盤點時間 (UTC)** | 2026-05-15 |
| **分析方式** | `npm run analyze-access -- "/nas/isin/workout.mdb" --json --no-samples`；含樣本：`--json --sample-offset=0 --sample-limit=15`；字串經腳本 Big5→UTF-8 轉換。 |
...
```bash
npm run analyze-access -- "/nas/isin/workout.mdb" --json --no-samples
npm run analyze-access -- "/nas/isin/workout.mdb" --json --sample-limit=20
```
