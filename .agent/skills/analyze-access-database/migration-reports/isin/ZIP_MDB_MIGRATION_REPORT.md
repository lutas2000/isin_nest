# zip.mdb — Legacy Access 遷移盤點報告

| 項目 | 內容 |
| --- | --- |
| **權威檔案路徑** | `/nas/isin/zip.mdb`（庫存「同名檔主檔判定」主檔相對路徑：`zip.mdb`；歷史盤點檔名對應 `isin/zip.mdb`） |
| **盤點時間 (UTC)** | 2026-05-15 |
| **分析方式** | `npm run analyze-access -- "/nas/isin/zip.mdb" --json --no-samples`；含樣本：`--json --sample-offset=0 --sample-limit=15`；字串經腳本 Big5→UTF-8 轉換。 |
...
| `zipname` | text | 20 | Y | varchar(20) | 郵遞區號 |
...
- **`zip`**：
  - 列 0: zipno=100; zipname=台北市中正區
  - 列 1: zipno=103; zipname=台北市大同區
  - 列 2: zipno=104; zipname=台北市中山區
  - 列 3: zipno=105; zipname=台北市松山區
  - 列 4: zipno=106; zipname=台北市大安區
...
```bash
npm run analyze-access -- "/nas/isin/zip.mdb" --json --no-samples
npm run analyze-access -- "/nas/isin/zip.mdb" --json --sample-limit=20
```
