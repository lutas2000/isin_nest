# phrase.mdb — Legacy Access 遷移盤點報告

| 項目 | 內容 |
| --- | --- |
| **權威檔案路徑** | `/nas/isin/phrase.mdb`（庫存「同名檔主檔判定」主檔相對路徑：`phrase.mdb`；歷史盤點檔名對應 `isin/phrase.mdb`） |
| **盤點時間 (UTC)** | 2026-05-15 |
| **分析方式** | `npm run analyze-access -- "/nas/isin/phrase.mdb" --json --no-samples`；含樣本：`--json --sample-offset=0 --sample-limit=15`；字串經腳本 Big5→UTF-8 轉換。 |
...
| --- | --- | ---: | --- | --- | --- |
| `phno` | text | 6 | Y | varchar(6) | 不明（建議對照舊程式與業務） |
| `phrase` | text | 100 | Y | varchar(100) | 不明（建議對照舊程式與業務） |
...
- **`phrase`**：
  - 列 0: phno=0; phrase=Θ
  - 列 1: phno=00; phrase=Θ*Θ*PCDΘ*Θ孔
  - 列 2: phno=000; phrase=Θ*Θ
  - 列 3: phno=001; phrase=折,
  - 列 4: phno=002; phrase=焊,
...
```bash
npm run analyze-access -- "/nas/isin/phrase.mdb" --json --no-samples
npm run analyze-access -- "/nas/isin/phrase.mdb" --json --sample-limit=20
```
