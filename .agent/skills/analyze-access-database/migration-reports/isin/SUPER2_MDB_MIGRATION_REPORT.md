# super2.mdb — Legacy Access 遷移盤點報告

| 項目 | 內容 |
| --- | --- |
| **權威檔案路徑** | `/nas/isin/super2.mdb`（庫存「同名檔主檔判定」主檔相對路徑：`super2.mdb`；歷史盤點檔名對應 `isin/super2.mdb`） |
| **盤點時間 (UTC)** | 2026-05-15 |
| **分析方式** | `npm run analyze-access -- "/nas/isin/super2.mdb" --json --no-samples`；含樣本：`--json --sample-offset=0 --sample-limit=15`；字串經腳本 Big5→UTF-8 轉換。 |
...
- **`itable`**：
  ...
  - 列 1: graph=POINT; sn=2
  - 列 2: graph=TEXT; sn=18
  - 列 3: graph=INSERT; sn=6
  - 列 4: graph=POLYLINE; sn=7
- **`jtable`**：
...
```bash
npm run analyze-access -- "/nas/isin/super2.mdb" --json --no-samples
npm run analyze-access -- "/nas/isin/super2.mdb" --json --sample-limit=20
```
