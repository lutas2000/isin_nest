# inventory.mdb — Legacy Access 遷移盤點報告

| 項目 | 內容 |
| --- | --- |
| **權威檔案路徑** | `/nas/isin/isin/inventory.mdb`（庫存「同名檔主檔判定」主檔相對路徑：`isin/inventory.mdb`；歷史盤點檔名對應 `isin/inventory.mdb`） |
| **盤點時間 (UTC)** | 2026-05-15 |
| **分析方式** | `npm run analyze-access -- "/nas/isin/isin/inventory.mdb" --json --no-samples`；含樣本：`--json --sample-offset=0 --sample-limit=15`；字串經腳本 Big5→UTF-8 轉換。 |
...
| `BENCH` | text | 2 | Y | varchar(2) | 無樣本列，需對照程式 |
...
| `ORDER_NO` | text | 10 | Y | varchar(10) | 無樣本列，需對照程式 |
| `DWG_NO` | text | 40 | Y | varchar(40) | 無樣本列，需對照程式 |
...
| `OSP` | text | 20 | Y | varchar(20) | 無樣本列，需對照程式 |
| `MK_NO` | text | 10 | Y | varchar(10) | 無樣本列，需對照程式 |
...
```bash
npm run analyze-access -- "/nas/isin/isin/inventory.mdb" --json --no-samples
npm run analyze-access -- "/nas/isin/isin/inventory.mdb" --json --sample-limit=20
```
