# plan.mdb — Legacy Access 遷移盤點報告

| 項目 | 內容 |
| --- | --- |
| **權威檔案路徑** | `/nas/isin/plan.mdb`（庫存「同名檔主檔判定」主檔相對路徑：`plan.mdb`；歷史盤點檔名對應 `isin/plan.mdb`） |
| **盤點時間 (UTC)** | 2026-05-15 |
| **分析方式** | `npm run analyze-access -- "/nas/isin/plan.mdb" --json --no-samples`；含樣本：`--json --sample-offset=0 --sample-limit=15`；字串經腳本 Big5→UTF-8 轉換。 |
...
| --- | --- | ---: | --- | --- | --- |
| `qno` | text | 15 | Y | varchar(15) | 報價／訂單單號類鍵 |
...
```bash
npm run analyze-access -- "/nas/isin/plan.mdb" --json --no-samples
npm run analyze-access -- "/nas/isin/plan.mdb" --json --sample-limit=20
```
